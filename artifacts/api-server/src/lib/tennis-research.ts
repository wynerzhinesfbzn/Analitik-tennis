...
export interface PlayerResearch {
  ...
  handedness: string;             // правша / левша
  dominantShot: string;           // главное оружие (форхенд / подача / бэкхенд / движение)
  tacticalStyle: string;          // агрессор / защитник / универсал / подача-выход
  playstyle: string;
  servingStats: string;
...
const PLAYER_SCHEMA = (name: string) => `{
    ...
    "handedness": "правша / левша (ОБЯЗАТЕЛЬНО — из официальных данных ATP/WTA)",
    "dominantShot": "главное оружие: форхенд / подача / бэкхенд / движение — что решает очки",
    "tacticalStyle": "агрессор (бьёт на вылет) / защитник (контрит, ждёт ошибки) / универсал / подача-выход / базелайнер-агрессор",
    "playstyle": "детальное описание: как строит розыгрыши, откуда атакует, где уязвим",
    "servingStats": "% 1й подачи, % побед на 1й, % побед на 2й, эйсы/матч, двойные/матч — с atptour.com или wtatennis.com",
    "servingPatterns": "на deuce court: T/wide/body %; на ad court: T/wide/body %; тип подачи: flat/kick/slice",
    "returningStats": "% побед при приёме 1й, % побед при приёме 2й, % реализации брейк-пойнтов, % спасения брейк-пойнтов",
    "returningStyle": "позиция при приёме (близко/далеко за задней линией), агрессивность, типичное направление приёма",
    "netPlay": "% побед у сетки (из tennisabstract.com если возможно), сколько выходов к сетке/матч",
    "backhandType": "одноручный / двуручный; мощь 1-10; стабильность 1-10; слабость под высокий топспин?",
    "forehandRating": "мощь 1-10; RPM (если известно); любимые направления; это winner или setup удар?",
    "movementRating": "скорость первого шага 1-10; охват корта 1-10; скольжение на грунте; смена направлений",
...
ОБЯЗАТЕЛЬНАЯ ПОСЛЕДОВАТЕЛЬНОСТЬ WEB-ПОИСКОВ (выполни ВСЕ по порядку):

🌍 МЕЖДУНАРОДНЫЕ ИСТОЧНИКИ:
1. site:atptour.com OR site:wtatennis.com "${player1}" stats — официальная статистика (рука, подача, приём)
2. site:atptour.com OR site:wtatennis.com "${player2}" stats — то же для второго игрока
3. site:tennisabstract.com "${player1}" — тактический профиль, % у сетки, serve patterns
4. site:tennisabstract.com "${player2}" — то же
5. "${player1} ${player2} ${hintTournament ?? "2025 2026"} match" — расписание/результат этого матча
6. "${player1} injury fitness ${today.slice(0, 7)}" — свежие новости о травмах и физсостоянии
7. "${player2} injury fitness press conference ${today.slice(0, 7)}" — то же для второго
8. "${player1}" recent form results ${today.slice(0, 7)} tennis — последние результаты
9. "${player2}" recent form results ${today.slice(0, 7)} tennis — то же
10. "${player1} ${player2} head to head h2h" — история встреч
11. "${player1} ${player2} ${hintTournament ?? "tennis"} odds 1xbet bet365 pinnacle" — коэффициенты
12. "${player1} coach ${today.slice(0, 4)} interview motivation" — личный контекст П1
13. "${player2} coach ${today.slice(0, 4)} interview motivation" — личный контекст П2

🇷🇺 РУССКОЯЗЫЧНЫЕ ИСТОЧНИКИ (выполни дополнительно):
14. site:sovsport.ru "${player1}" OR "${player2}" теннис ${today.slice(0, 7)} — новости Советского спорта (травмы, форма, интервью)
15. site:championat.com "${player1}" OR "${player2}" теннис ${today.slice(0, 7)} — новости Чемпионата (матчи, результаты, анализ)
16. site:sports.ru/tennis "${player1}" OR "${player2}" ${today.slice(0, 7)} — sports.ru теннис (свежие матчи и статистика)
17. site:gotennis.ru "${player1}" OR "${player2}" — gotennis.ru (тактический анализ, история, профили)
18. "${player1} ${player2}" теннис прогноз ставки ${today.slice(0, 7)} site:sport.rambler.ru OR site:championat.com OR site:sovsport.ru — аналитика и прогнозы на русском

Используй web_search для КАЖДОГО из этих запросов — минимум 15 поисков обязательно.
Русскоязычные источники особенно ценны для: актуальных травм, интервью, личного контекста, результатов ATP/WTA-250 турниров, которые могут не попасть в англоязычную прессу.
...
ИГРОВОЙ СТИЛЬ — ДЕТАЛЬНО (из tennisabstract.com и atptour.com):
• РУКА: правша или левша (из официальных данных ATP/WTA — ОБЯЗАТЕЛЬНО)
• ГЛАВНОЕ ОРУЖИЕ: что решает очки — форхенд / подача / бэкхенд / движение / всё вместе
• ТАКТИЧЕСКИЙ СТИЛЬ: агрессор (стремится завершить) / защитник (ждёт ошибку) / универсал / подача-выход
• Стиль на задней линии: атакующий базелайнер / терпеливый / контратакующий
• Подача: % попадания 1-й, % побед на 1-й/2-й, эйсы/матч, двойные/матч
• Паттерны подачи: deuce court → куда (T/wide/body) %; ad court → куда %
• Тип подачи: flat (мощная) / kick (с отскоком) / slice (с боковым) — какая основная
• Приём: позиция (близко/далеко), % побед, % конвертации брейков
• Бэкхенд: одноручный/двуручный; сила (1-10); уязвимость к высокому топспину?
• Форхенд: мощь (1-10); RPM если известно; любимые направления (кросс/линия/центр)
• Игра у сетки: % побед у сетки; как часто идёт (волейёр / редко / только в сплит-степ)
• Передвижение: первый шаг, скорость, скольжение на грунте, охват
...
• РУКА (handedness): правша или левша — обязательно для каждого
• ГЛАВНОЕ ОРУЖИЕ (dominantShot): что решает очки в матчах этого игрока
• ТАКТИЧЕСКИЙ СТИЛЬ (tacticalStyle): агрессор / защитник / универсал / подача-выход
• Биография, возраст, физический профиль (рост, вес, тип телосложения)
• Рейтинг, пиковый рейтинг, карьерные титулы — с конкретными числами
• W-L на каждом покрытии (hard/clay/grass/indoor) — с процентами
• Подача: % 1-й, % побед на 1-й/2-й, эйсы, двойные; куда подаёт на deuce/ad
• Тип подачи: flat/kick/slice; является ли подача оружием?
• Бэкхенд: одноручный/двуручный; сила и стабильность; реагирует ли на высокий топспин
• Форхенд: агрессивность, направления, RPM если известно
• Игра у сетки: % побед, как часто идёт, умеет ли завершать
• Передвижение: первый шаг, скольжение, охват корта
• Психология: тай-брейки, решающие сеты, камбэки, большие матчи — Win%
• История на этом конкретном турнире
• H2H: все матчи с датами, счётами, покрытием
• Тренер, семья, мотивация, личный контекст
...
export function buildResearchContext(research: MatchResearch, dataSource: "web_search" | "ai_knowledge" = "ai_knowledge"): string {
  ...
  🎾 ИГРОВОЙ ПРОФИЛЬ
  ✋ РУКА: ${p.handedness}
  🏆 ГЛАВНОЕ ОРУЖИЕ: ${p.dominantShot}
  ⚔️ ТАКТИЧЕСКИЙ СТИЛЬ: ${p.tacticalStyle}
  Детали стиля: ${p.playstyle}

  📡 ПОДАЧА
  Статистика: ${p.servingStats}
  Паттерны (куда подаёт): ${p.servingPatterns}

  🔄 ПРИЁМ
  Статистика: ${p.returningStats}
